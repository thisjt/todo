...
export class AuthenticationService implements IAuthenticationService {
	...
	createToken(session: Session): string {
		return jwt.sign(session, this._context.env.JWT_SECRET);
	}

	validateSession(token: string): Session | null {
		const verified = jwt.verify(token, this._context.env.JWT_SECRET) as typeof Session.prototype;

		if (!verified) return null;

		return new Session(verified);
	}
